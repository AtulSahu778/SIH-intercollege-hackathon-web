  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {/* Category chart */}
        <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
          <h2 className="font-bold text-text-primary mb-5">Category Distribution</h2>
          <div className="flex gap-6">
            {categoryData.map((cat, i) => (
              <div key={cat.name} className="flex-1 text-center p-4 rounded-xl bg-slate-50 border border-slate-100">
                <div className="text-2xl font-black text-text-primary">{cat.count}</div>
                <div className="text-xs text-text-muted mt-1">{cat.name}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              <select
                ...
                className="h-9 px-3 rounded-xl border border-slate-200 text-sm bg-white text-text-primary focus:outline-none focus:ring-2 focus:ring-accent-orange/30"
              >
                ...
                <tbody className="divide-y divide-slate-50">
                  {filtered.map((reg) => (
                    <tr key={reg.teamId} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-4 py-3 text-text-muted hidden md:table-cell max-w-[140px] truncate text-xs">{reg.department}</td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-8 w-8 text-text-muted hover:text-navy-primary"
                            onClick={() => setSelectedReg(reg)}
                            aria-label={`View details for ${reg.teamName}`}
                          >
                          ...
                          {reg.status !== "Approved" && (
                            <Button
                              ...
                              disabled={updatingId === reg.teamId}
            ...
            <>
              <DialogHeader className="pb-0">
                <div className="flex items-center gap-3">
                  ...
                  <Badge variant={statusBadgeVariant(selectedReg.status as RegistrationStatus)} className="ml-auto">
                    {selectedReg.status}
                  </Badge>
                </div>
              </DialogHeader>
