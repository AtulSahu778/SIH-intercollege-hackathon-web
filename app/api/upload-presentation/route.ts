...
const TIMEOUT_MS = 120_000; // 2 min — PPT files can be large

export async function POST(req: NextRequest) {
  try {
    const body = await req.json();
    const { teamId, base64, fileName, mimeType } = body;

    if (!teamId || !base64 || !fileName) {
      return NextResponse.json(
        { success: false, error: "teamId, base64, and fileName are required." },
        { status: 400 }
      );
    }
    ...
    if (!APPS_SCRIPT_URL || APPS_SCRIPT_URL.includes("YOUR_SCRIPT_ID")) {
      return NextResponse.json({
        success: true,
        message: "Presentation uploaded successfully (mock mode).",
      });
    }
    ...
    if (result === "timeout") {
      return NextResponse.json({
        success: true,
        message: "Your presentation has been submitted and is being processed.",
      });
    }
    ...
    if (
      ...
      console.error("Apps Script upload-presentation error:", result.text.slice(0, 200));
      return NextResponse.json(
